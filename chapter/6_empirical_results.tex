...
The estimates in Table~\ref{tab:baseline_did} compare three baseline DID models. Column~(1) features firm and year fixed effects but no time-varying controls; the coefficient on MSCI $\times$ Post is 0.705 ($\mathrm{s.e.} = 0.038$, $p < 0.001$), which translates to $\exp(0.705) - 1 \approx 102\%$ increase in the log-transformed digital transformation index.

Adding rich time-varying controls in Column~(2) reduces the magnitude to 0.394 ($\mathrm{s.e.} = 0.039$, $p < 0.001$). We consider Column~(2) our preferred specification because it balances covariate adjustment with the preservation of treatment variation; $\exp(0.394) - 1 \approx 48\%$. Column~(3) further enhances the baseline with industry$\times$year and province$\times$year fixed effects, resulting in a smaller, statistically insignificant coefficient (0.044; $\mathrm{s.e.} = 0.030$). We interpret Column~(3) as a conservative lower bound: since MSCI eligibility correlates with industry composition, interactive fixed effects might absorb treatment-induced variation, indicating over-controlling rather than evidence of a null effect.
...
Before analyzing formal regression evidence, it is helpful to examine the raw time-series trajectories of digital transformation for both treated and control firms.
...
While Figure~\ref{fig:event_study} provides visual confirmation that the two groups diverge after 2018, it does not formally distinguish between a genuine post-treatment shift and a pre-existing differential trend. The event-study regression in Equation~(\ref{eq:event_study}) addresses this by estimating year-specific treatment coefficients $\delta_k$, each expressed relative to the omitted reference year $k = -1$.
...
The matched-sample evidence in Table~\ref{tab:psm_did} confirms the same direction as the baseline model. Columns~(1) and~(2) implement PSM-DID using overall and year-by-year matching; both yield positive MSCI $\times$ Post coefficients consistent with the baseline. Dividing by the control mean (12.78) yields a PSM-implied effect of approximately 15--20\%, comparable to the log-point estimate accounting for the log transformation.

\subsection{Alternative Measures of Digital Transformation}
\label{sec:alt_measures}

A key vulnerability of any text-based outcome measure is sensitivity to the particular choice of operationalization. Eight alternative digital transformation proxies are examined across four panels.
...
\toprule
\textbf{Panel} & \textbf{Variable} & \textbf{Coeff.} & \textbf{Std.\ Err.} & \textbf{$N$} & \textbf{$R^2$} \\
\midrule
A: Comprehensive & DT\_Comp\_A & 2.097*** & 0.607 & 35,059 & 0.034 \\
A: Comprehensive & DT\_Comp\_B & 5.814*** & 1.276 & 35,059 & 0.058 \\
A: Comprehensive & DT\_App\_A & 0.634** & 0.304 & 35,059 & 0.013 \\
A: Comprehensive & DT\_App\_B & 2.494*** & 0.630 & 35,059 & 0.044 \\
B: Tech-Specific & AI\_Tech & 0.747*** & 0.195 & 35,059 & 0.028 \\
B: Tech-Specific & BigData\_Tech & 0.396** & 0.160 & 35,059 & 0.016 \\
B: Tech-Specific & Cloud\_Tech & 0.308 & 0.196 & 35,059 & 0.018 \\
B: Tech-Specific & Blockchain\_Tech & 0.013 & 0.038 & 35,059 & 0.010 \\
C: Alt.\ Operationalizations & Binary (Above Median) & 0.021*** & 0.006 & 35,264 & 0.091 \\
C: Alt.\ Operationalizations & Standardized (Z-score) & 0.063*** & 0.018 & 35,059 & 0.034 \\
D: Mentions vs.\ Intensity & BigData\_Mention & 0.204 & 0.138 & 35,059 & 0.015 \\
D: Mentions vs.\ Intensity & Cloud\_Mention & 0.500*** & 0.155 & 35,059 & 0.008 \\
D: Mentions vs.\ Intensity & Blockchain\_Mention & 0.005 & 0.036 & 35,059 & 0.009 \\
D: Mentions vs.\ Intensity & DigCurrency & 0.005 & 0.007 & 35,059 & 0.003 \\
D: Mentions vs.\ Intensity & DigFinance & 0.029* & 0.015 & 35,059 & 0.002 \\
D: Mentions vs.\ Intensity & DigMarketing & $-$0.050*** & 0.015 & 35,059 & 0.001 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes:} Units vary by panel. Panel~A and B coefficients are keyword count units (level-form); divide by 12.78 for percentage effects. Panel~C uses binary (0/1) or standardized Z-scores. Panel~D uses mention frequency counts (level-form). All regressions include firm and year fixed effects with full time-varying controls. Standard errors clustered at firm level. Significance: *** $p < 0.01$, ** $p < 0.05$, * $p < 0.10$.
\end{tablenotes}
\end{table}
...
Recent advances in DID econometrics have demonstrated that conventional TWFE estimators may produce biased averages when treatment is staggered across time and treatment effects differ across cohorts~\cite{GoodmanBacon2021}. As a diagnostic check, cohort-specific average treatment effects on the treated (ATTs) are estimated by comparing each inclusion cohort separately against the never-treated firm subsample.
...
Table~\ref{tab:cohort_att} reports cohort-specific ATTs. The 2018 cohort (151 firms) shows ATT = 0.0492; the 2019 cohort (223 firms) shows ATT = 0.0135. These estimates are directionally consistent with the pooled TWFE result but smaller due to the conservative FE structure.

A more demanding alternative adds firm-level fixed effects on top of year effects:
...
Table~\ref{tab:cohort_att_firm} shows estimates of 0.0612 (2018 cohort) and $-$0.0421 (2019 cohort), centered near zero. This attenuation under firm FE is consistent with our baseline interpretation---the high-dimensional firm FE absorbs cross-sectional treatment variation correlated with firm-specific trends.

\subsection{Disclosure-Length Controls and Length-Normalized Outcome Specifications}

One concern specific to text-based outcome measures is that MSCI inclusion may incentivize firms to lengthen their annual reports in response to international investor demand for richer disclosure, mechanically inflating keyword counts.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Specification} & \textbf{Coeff.} & \textbf{Std.\ Err.} & \textbf{$P$-value} & \textbf{$N$} \\
\midrule
$\log(DT)$ + controls + FE & 0.0436 & 0.0496 & 0.3799 & 34,503 \\
$\log(DT)$ + controls + disclosure proxy + FE & 0.0656 & 0.0494 & 0.1840 & 34,503 \\
$\log(DT/\mathrm{1k\text{-}chars})$ + controls + FE & 0.0643 & 0.0377 & 0.0882 & 34,503 \\
$\log(DT/\mathrm{1k\text{-}chars})$ + controls + disc.\ proxy + FE & 0.0516 & 0.0375 & 0.1694 & 34,503 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes:} All specifications include Industry, Year, and Province fixed effects with full time-varying controls and firm-clustered standard errors. The disclosure proxy is $\log(1 + \mathrm{total\ character\ count})$.
\end{tablenotes}
\end{table}

Table~\ref{tab:disclosure_length} addresses disclosure verbosity concerns. The positive direction is maintained across all four specifications, and the normalized outcome achieves marginal significance ($p = 0.088$). These results confirm that the treatment effect is not driven by mechanical increases in report length post-MSCI inclusion.

\subsection{Alternative Clustering Strategies and Sample Restriction Checks}

Treatment effects remain positive and statistically significant when clustering standard errors at the industry level, province level, and industry-year level. Excluding firms listed during or after 2016, excluding STAR Market-listed firms, or restricting to manufacturing industries all yield qualitatively consistent results.
...
\begin{tabular}{llcccc}
\toprule
\textbf{Panel} & \textbf{Group} & \textbf{Coeff.} & \textbf{Std.\ Err.} & \textbf{$p$-value} \\
\midrule
A: Regional & East & 16.084*** & 2.125 & 0.000 \\
A: Regional & Central & 11.210*** & 1.559 & 0.000 \\
A: Regional & West & 6.185*** & 1.955 & 0.002 \\
B: Industry \& Tech & Industry C39 (Instrumentation) & 28.130*** & 5.201 & 0.000 \\
B: Industry \& Tech & Industry C38 (Electrical Equip.) & 13.032*** & 2.968 & 0.000 \\
B: Industry \& Tech & Industry C27 (Transport Equip.) & 2.764*** & 0.538 & 0.000 \\
B: Industry \& Tech & High- vs Low-Tech Differential & 50.589** & 23.944 & 0.035 \\
B: Industry \& Tech & High- vs Low-Competition Diff. & $-$10.071*** & 3.847 & 0.009 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes:} Dependent variable is DT\_Comp\_A (keyword counts, level-form). Coefficients are in keyword count units; divide by 12.78 for percentage effects. All coefficients from interaction regressions augmenting Equation~(\ref{eq:twfe_did}). Firm and year fixed effects and the full control set are included. Standard errors are firm-clustered. Significance: *** $p < 0.01$, ** $p < 0.05$, * $p < 0.10$.
\end{tablenotes}
\end{table}
...
\toprule
\textbf{Panel} & \textbf{Dimension} & \textbf{Coeff.} & \textbf{Std.\ Err.} & \textbf{$p$} & \textbf{$N$} \\
\midrule
A: Industry & Manufacturing (C) & 7.318*** & 0.762 & 0.000 & 24,032 \\
A: Industry & Information Services (I) & $-$8.007 & 7.311 & 0.274 & 2,520 \\
A: Industry & Finance (F) & $-$4.056 & 4.185 & 0.333 & 1,771 \\
A: Industry & Utilities (D) & 0.431 & 0.664 & 0.517 & 1,125 \\
A: Industry & Joint Test ($F$-stat) & \multicolumn{2}{c}{97.26 ($p < 0.001$)} & & 35,059 \\
B: Regional & Eastern Provinces & 16.084*** & 2.125 & 0.000 & 17,334 \\
B: Regional & Central Provinces & 11.210*** & 1.559 & 0.000 & 8,828 \\
B: Regional & Western Provinces & 6.185*** & 1.955 & 0.002 & 8,442 \\
B: Regional & Regional Interaction & 10.446*** & 2.403 & 0.000 & 34,604 \\
C: Tech Subcategories & High-End Manufacturing (C3) & 12.638*** & 1.168 & 0.000 & 14,019 \\
C: Tech Subcategories & Basic Manufacturing (C2) & 0.930 & 0.592 & 0.117 & 6,952 \\
C: Tech Subcategories & Equipment Manufacturing (C1) & 1.092 & 1.129 & 0.333 & 2,322 \\
C: Tech Subcategories & Cross-Category Test & \multicolumn{2}{c}{147.21 ($p < 0.001$)} & & 35,059 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes:} Panel~A: industry-level effects using CSRC classifications. Panel~B: regional analysis. Panel~C: manufacturing subcategories by technology intensity. Pre-treatment control mean = 12.78 keywords. All specifications include firm and year FE and full controls. Significance: *** $p < 0.01$, ** $p < 0.05$, * $p < 0.10$.
\end{tablenotes}
\end{table}
...
The financing constraint channel proposes that MSCI inclusion lowers the cost of external capital for treated firms.

To make this channel operational, the thesis uses three complementary proxies. First, the KZ Index captures broad financing frictions, with higher values indicating tighter constraints. Following standard corporate-finance practice, the index combines information on cash flow, leverage, valuation, cash holdings, and payout behavior. Second, Bank Leverage measures reliance on bank debt financing relative to firm scale; a decline after treatment is consistent with reduced dependence on potentially costly or restrictive bank credit. Third, CashFlow/Assets measures internally generated liquidity available for investment. All three mechanism regressions use the same identification structure as the baseline model (Industry+Year+Province fixed effects with firm-clustered standard errors), so channel estimates are directly comparable to the main DID specification.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Variable} & \textbf{Coeff.} & \textbf{(SE)} & \textbf{$p$-val} & \textbf{$N$} \\
\midrule
KZ Index (financing constraint) & $-$0.0385 & 0.0733 & 0.5992 & 28,339 \\
Bank Leverage (bank debt reliance) & $-$0.0318*** & 0.0027 & 0.000 & 23,552 \\
CashFlow / Assets (operating CF) & 0.0201*** & 0.0035 & 0.000 & 34,770 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes:} All regressions employ Industry+Year+Province fixed effects with firm-clustered standard errors. $N$ varies by proxy due to data availability in CSMAR. Significance: *** $p < 0.01$.
\end{tablenotes}
\end{table}

The financing results indicate partial but economically coherent support for this channel. The KZ Index coefficient is negative ($-$0.0385) but statistically insignificant ($p = 0.599$), suggesting that broad composite-constraint measures do not move precisely enough in this sample to identify a strong treatment response. By contrast, Bank Leverage declines significantly ($-$0.0318***, $p < 0.001$), with a tight confidence interval implied by the small standard error, indicating a clear post-inclusion reduction in bank-debt dependence. CashFlow/Assets increases significantly ($+$0.0201***, $p < 0.001$), showing stronger internal funding capacity after treatment. Jointly, these estimates are consistent with financing-friction relaxation operating through concrete balance-sheet and liquidity margins even when the omnibus KZ composite is noisy.
...
This channel is measured using board structure and ownership concentration variables that map directly to monitoring intensity. $N$(Independent Directors) is the count of independent directors on the board and captures board-level monitoring capacity in levels. Independent Director Ratio (\%) scales that count by board size and captures governance composition intensity. Top-10 Ownership HHI is constructed as a Herfindahl-type concentration index over top-shareholder holdings; lower values indicate more dispersed ownership and potentially broader external oversight. Estimation again uses the same DID framework and fixed-effect structure as the baseline model, ensuring interpretation consistency across channels.
...
\textbf{Variable} & \textbf{Coeff.} & \textbf{(SE)} & \textbf{$p$-val} & \textbf{$N$} \\
\midrule
$N$(Independent Directors) & 0.0518*** & 0.0152 & 0.0007 & 26,522 \\
Independent Director Ratio (\%) & 1.8238*** & 0.5627 & 0.0012 & 20,136 \\
Top-10 Ownership HHI (concentration) & $-$0.0302*** & 0.0054 & 0.000 & 34,753 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes:} All three governance proxies significant at $p < 0.01$. The HHI result (negative coefficient = less concentrated = better-governed) is directionally consistent with governance improvement theory. Standard errors clustered at firm level. Industry+Year+Province FE throughout.
\end{tablenotes}
\end{table}

Governance estimates are consistently in the predicted direction and statistically significant. MSCI-included firms add independent directors ($+$0.0518***, $p = 0.0007), and the independent-director share rises by 1.8238 percentage points ($p = 0.0012), indicating that the effect is not only mechanical board expansion but also a compositional shift toward independent oversight. The Top-10 Ownership HHI falls significantly ($-$0.0302***, $p < 0.001$), implying less concentrated control and stronger scope for minority-shareholder monitoring. Because all three governance proxies move in the theory-consistent direction and are statistically significant at conventional levels, the governance channel is supported more strongly than the financing channel in this dataset.
...
\begin{tablenotes}
\small
\item \textit{Notes:} This table reports identification requirements for a direct knowledge-spillover test and does not present regression coefficients. The deferred strategy is intentional: the thesis avoids proxy substitution that could weaken causal interpretation until direct mediators are integrated into the merged panel.
\end{tablenotes}
\end{table}
