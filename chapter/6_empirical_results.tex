...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig6_5_mechanism_pathway.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig6_6_mechanism_channels.png}
...
The financing constraint channel predicts that MSCI inclusion relaxes financial constraints. Bank Leverage (coefficient = $-$0.0318***, $p < 0.001$) indicates that MSCI-included firms reduce reliance on costly bank debt. CashFlow/Assets (coefficient = $+$0.0201***, $p < 0.001$) documents improved internal cash flow generation following MSCI inclusion.
...
MSCI-included firms add more independent directors ($+$0.0518***, $p = 0.001$), increase the share of independent directors ($+$1.8238***, $p = 0.001$), and experience less concentrated ownership ($-$0.0302***, $p < 0.001$), consistent with dispersed monitoring and stronger minority-shareholder oversight.

\subsection{Knowledge Spillover Channel: Deferred Direct Test and Data Requirements}

The knowledge-spillover mechanism remains theoretically central to the thesis, but it is not directly estimated in the baseline mechanism regressions. A direct mechanism test requires mediator variables that map to the chain MSCI inclusion $\to$ external information intermediaries/investors $\to$ firm learning and implementation. In this context, analyst coverage (and optionally analyst forecast dispersion) and foreign institutional ownership are the preferred first-stage mediators. Because these variables are not yet integrated into the current merged panel, this thesis does not report a direct knowledge-channel coefficient and avoids proxy substitution that could blur causal interpretation.

\begin{table}[htbp]
\centering
\caption{Mechanism Analysis: Knowledge Spillover Channel (Deferred)}
\label{tab:knowledge_deferred}
\scriptsize
\setlength{\tabcolsep}{4pt}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}Xcccccc>{\raggedright\arraybackslash}X}
\toprule
\textbf{Variable} & \textbf{Coeff.} & \textbf{SE} & \textbf{$p$-value} & \textbf{$N$} & \textbf{$R^2$} & \textbf{$N_{\text{treated}}$} & \textbf{Direction Note} \\
\midrule
...
\begin{tablenotes}
\small
\item \textit{Notes:} This table documents deferred status rather than an estimated regression result. Direct mediator variables (analyst coverage, foreign institutional ownership, and cross-border exposure) are not yet integrated in the merged panel.
\end{tablenotes}
\end{table}
...
Knowledge (Deferred) & Direct mediators not in panel & NA & NA & NA & NA \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Notes:} All regressions use Industry+Year+Province FE with firm-clustered standard errors. Across the six directly tested proxies, five are significant at the 1\% level and all are directionally consistent with theory.
\end{tablenotes}
\end{table}
