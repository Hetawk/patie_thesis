...
The matched-sample evidence in Table~\ref{tab:psm_did} is directionally consistent with the baseline model but less precise. Columns~(1) and~(2) implement PSM-DID using overall and year-by-year matching; both MSCI $\times$ Post coefficients are positive, but only Column~(2) is statistically significant. Relative to the control mean (12.78), the level-form magnitudes imply roughly 15--21\% effects, which should be interpreted as supportive sensitivity evidence rather than as a replacement for the preferred baseline estimate.
...
The financing estimates show an internally consistent pattern once variable scaling is accounted for. The KZ coefficient is negative ($-$0.0385) but imprecise (SE $=0.0733$, $p = 0.599$), so the broad composite index does not provide statistically reliable movement in this sample. By contrast, Bank Leverage declines significantly ($-$0.0318***, $p < 0.001$); on a ratio scale this is approximately a 3.2 percentage-point reduction in bank-debt reliance after inclusion. CashFlow/Assets rises by 0.0201*** ($p < 0.001$), equivalent to roughly a 2.0 percentage-point increase in internally generated funding capacity. The contrast between the noisy composite index and the precise balance-sheet/liquidity proxies is informative rather than contradictory: financing easing appears strongest in directly measured funding margins.
...
Governance estimates are consistently in the predicted direction and statistically significant. MSCI-included firms add independent directors ($+$0.0518***, $p = 0.0007$); while the per-firm annual increment is modest, the estimate is precise across a large sample ($N=26{,}522$). More importantly, the independent-director share rises by 1.8238 percentage points ($p = 0.0012$), showing that the result is not merely a larger board but a higher governance-independence share. The Top-10 Ownership HHI declines ($-$0.0302***, $p < 0.001$), implying measurable ownership dispersion and broader scope for minority-shareholder oversight. Taken jointly, these three proxies provide convergent evidence that the governance channel is both statistically stable and economically interpretable in this dataset.
...
To keep this deferred channel analytically transparent, Table~\ref{tab:knowledge_deferred} reports a structured measurement-and-identification roadmap rather than a placeholder pseudo-regression.
