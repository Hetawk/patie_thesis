...
\midrule
\textbf{Variable} & \textbf{(9) Binary} & \textbf{(10) Z-score} &  &  \\
\midrule
MSCI $\times$ Post & 0.021*** (0.006) & 0.063*** (0.018) &  &  \\
Firm-FE & Yes & Yes &  &  \\
Year-FE & Yes & Yes &  &  \\
Controls & Yes & Yes &  &  \\
$N$ & 35,264 & 35,059 &  &  \\
...
\begin{tabular}{lcccc}
\toprule
\textbf{Variable} & \textbf{(1) Log DT} & \textbf{(2) Log DT + Length} & \textbf{(3) Log DT/1k} & \textbf{(4) Log DT/1k + Length} \\
\midrule
...
\toprule
\multicolumn{6}{c}{\textbf{Panel A: Regional Subsamples}} \\
\midrule
\textbf{Variable} & \textbf{(1) East} & \textbf{(2) Central} & \textbf{(3) West} & \multicolumn{2}{c}{} \\
\midrule
MSCI $\times$ Post & 16.084*** (2.125) & 11.210*** (1.559) & 6.185*** (1.955) & \multicolumn{2}{c}{} \\
Firm-FE & Yes & Yes & Yes & \multicolumn{2}{c}{} \\
Year-FE & Yes & Yes & Yes & \multicolumn{2}{c}{} \\
Controls & Yes & Yes & Yes & \multicolumn{2}{c}{} \\
$N$ & 17,334 & 8,828 & 8,442 & \multicolumn{2}{c}{} \\
\midrule
...
\midrule
\multicolumn{5}{c}{\textbf{Panel C: Manufacturing Technology Subcategories}} \\
\midrule
...
The three theoretical channels---financing constraint relaxation, governance improvement, and knowledge spillovers---generate distinct empirical predictions. This thesis directly tests the first two channels by estimating Equation~(\ref{eq:twfe_did}) with channel-specific proxy outcomes. For the knowledge-spillover channel, the current sample provides exploratory evidence with suggestive interpretation rather than direct-mediator coefficient estimates.
...
\subsection{Knowledge Spillover Channel: Exploratory Evidence}

The knowledge-spillover mechanism remains theoretically central to the thesis. Recent evidence shows that digital transformation can generate information spillovers across firms and market participants~\cite{WangWang2026}, and that spillover intensity may vary with strategic competition conditions in manufacturing settings~\cite{LiuGu2026}, reinforcing the conceptual relevance of this channel in our setting. In the current sample, this channel is assessed with exploratory evidence and interpreted as suggestive rather than definitive.

As a supplementary check, Appendix Table~\ref{tab:appendix_spillover_proxy} reports an exploratory peer-spillover proxy regression based on leave-one-out industry-year peer digitalization. The interaction term (MSCI $\times$ PeerDT) is positive but statistically insignificant ($p = 0.114$), so it is interpreted as suggestive rather than conclusive mechanism evidence.

Taken together, Section~6.5 provides direct mechanism evidence for financing and governance channels. For the knowledge-spillover channel, current evidence is exploratory and should be interpreted as suggestive rather than definitive.
...
\begin{tabular}{lccc}
\toprule
\multicolumn{4}{c}{\textbf{Panel A: Financing Proxies}} \\
\midrule
\textbf{Variable} & \textbf{(1) KZ Index} & \textbf{(2) Bank Leverage} & \textbf{(3) CashFlow/Assets} \\
\midrule
MSCI $\times$ Post & $-$0.0385 (0.0733) & $-$0.0318*** (0.0027) & 0.0201*** (0.0035) \\
...
$N$ & 28,339 & 23,552 & 34,770 \\
\midrule
\multicolumn{4}{c}{\textbf{Panel B: Governance Proxies}} \\
\midrule
\textbf{Variable} & \textbf{(4) Indep. Dir. Count} & \textbf{(5) Indep. Dir. Ratio} & \textbf{(6) Top-10 HHI} \\
\midrule
MSCI $\times$ Post & 0.0518*** (0.0152) & 1.8238*** (0.5627) & $-$0.0302*** (0.0054) \\
Industry-FE & Yes & Yes & Yes \\
Year-FE & Yes & Yes & Yes \\
...
\begin{tablenotes}
\small
\item \textit{Notes:} Entries report MSCI $\times$ Post coefficients with firm-clustered standard errors in parentheses. Panels~A and~B summarize directly estimated mechanisms under a common fixed-effect structure. Significance: *** $p < 0.01$, ** $p < 0.05$, * $p < 0.10$.
\end{tablenotes}
\end{table}
