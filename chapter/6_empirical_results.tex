...
The central empirical question is whether MSCI inclusion is associated with a post-inclusion increase in corporate digital transformation intensity under maintained DID assumptions. Equation~(\ref{eq:twfe_did}) is first reported as a TWFE benchmark using phased inclusion timing, with firm and year fixed effects absorbing persistent firm differences and common shocks. Two nested models are estimated: a parsimonious FE model and a controlled FE model with the full time-varying covariate set from Chapter~\ref{chap:research_design_1}. Because treatment adoption is staggered, these TWFE estimates are interpreted as benchmark evidence and later cross-checked against cohort-based modern DID estimators.
...
\begin{tablenotes}
\small
\item \textit{Notes:} Dependent variable is DT Index (log-transformed), $DT\_Comp = \log(1 + \Sigma\;\mathrm{keywords})$. Coefficients are in log points; to convert to percentage effects, use $\exp(\beta) - 1$. Column~(1) is the baseline without controls; Column~(2) is the controlled TWFE benchmark with full controls. Robust standard errors clustered at the firm level. Significance: *** $p < 0.01$, ** $p < 0.05$, * $p < 0.10$.
\end{tablenotes}
\end{table}

The estimates in Table~\ref{tab:baseline_did} indicate positive TWFE coefficients in both columns. Column~(1), with firm and year fixed effects only, yields 0.705 ($\mathrm{s.e.} = 0.038$, $p < 0.001$). Adding rich controls in Column~(2) reduces the estimate to 0.394 ($\mathrm{s.e.} = 0.039$, $p < 0.001$).

This attenuation pattern is informative, but TWFE magnitudes are not treated as definitive in this study because staggered adoption can induce weighting distortions when treatment effects are heterogeneous. Accordingly, the 0.394 estimate is interpreted as a benchmark for comparability with prior literature, while conservative magnitude interpretation is anchored in modern DID diagnostics in Section~6.3.5.

\subsection{Stability Across Specifications and Control Sets}

Key control-variable signs are broadly consistent with priors: larger firms display higher digital-intensity outcomes, while higher leverage is associated with lower digital intensity and stronger internal cash flow is associated with higher intensity. These patterns support model coherence, but they are interpreted as conditional associations rather than standalone mechanism confirmation.

\subsection{Economic Interpretation and Practical Significance}

At the control-group mean of 12.78 keywords, the TWFE benchmark estimate of 0.394 log points corresponds to approximately 6.0 additional digital keywords, moving a median treated firm from roughly the 50th to around the 60th percentile of the pre-treatment distribution. This is economically large under the TWFE metric. However, because modern DID estimates are materially smaller, practical interpretation in this thesis emphasizes a positive but moderate and heterogeneous effect rather than a uniformly large treatment response.
...
Table~\ref{tab:alt_measures} reports MSCI $\times$ Post coefficients for eight alternative digital-transformation outcome measures. Panel~A reports positive and significant effects for all four comprehensive variants. Panel~B indicates significant effects for AI (0.747***) and Big Data (0.396**). Panel~C confirms positive effects using binary (0.021***) and standardized Z-score (0.063***) specifications. These patterns support directional robustness, but they do not eliminate the disclosure-versus-implementation distinction discussed in Chapter~\ref{chap:research_design_2}; interpretation remains consistent with a hybrid symbolic-substantive measure.
...
Under this stricter specification (Table~\ref{tab:cohort_att_firm}), the 2018 cohort remains positive but imprecise (0.0612, s.e. = 0.0635, $p = 0.3347$; 95\% interval about [$-0.063$, $0.186$]), while the 2019 cohort turns negative but also imprecise ($-0.0421$, s.e. = 0.0564, $p = 0.4557$; 95\% interval about [$-0.153$, $0.068$]). The weighted average across 2018--2019 is essentially zero ($-0.0004$). Collectively, Tables~\ref{tab:cohort_att} and~\ref{tab:cohort_att_firm} are most informative as a sign-stability diagnostic against TWFE weighting artifacts, rather than as high-power evidence for precise cohort ranking. The divergence between TWFE (0.394) and cohort-based estimates is consistent with treatment-effect heterogeneity and potential negative-weighting issues emphasized in modern DID research. Substantively, the evidence supports a cautious interpretation: liberalization appears directionally favorable for digital-transformation outcomes, but average effects are concentrated, estimator-sensitive, and economically smaller than the conventional TWFE benchmark implies.
...
The financing evidence is mixed and should be interpreted cautiously. The KZ coefficient is negative ($-$0.0385) but imprecise (SE $=0.0733$, $p = 0.599$), so the composite financing-constraint index does not move significantly in this sample. In contrast, Bank Leverage declines significantly ($-$0.0318***, $p < 0.001$), equivalent to about a 3.2 percentage-point decline in bank-debt reliance, and CashFlow/Assets increases by 0.0201*** ($p < 0.001$), roughly a 2.0 percentage-point rise in internal liquidity.

Taken together, this pattern is more consistent with financing-structure adjustment and improved financing flexibility than with a broad uniform relaxation of underlying financing frictions. Lower bank leverage can reflect debt-equity rebalancing, and higher cash flow may partly capture jointly evolving operating performance. Therefore, the financing channel is supported directionally but interpreted as moderate rather than definitive.
...
Governance estimates are directionally consistent and statistically significant, but interpretation requires institutional caution. MSCI-included firms add independent directors ($+$0.0518***, $p = 0.0007$), and the independent-director ratio rises by 1.8238 percentage points ($p = 0.0012$). Top-10 Ownership HHI declines ($-$0.0302***, $p < 0.001$), indicating more dispersed ownership.

These patterns are consistent with stronger external monitoring and disclosure discipline, yet they do not by themselves prove deep internal governance transformation. In the Chinese setting, independent-director ratios are partly compliance-driven, and ownership dispersion can also reflect passive benchmark-tracking flows after index inclusion. Accordingly, the governance channel is interpreted as evidence of strengthened external oversight conditions rather than definitive governance optimization.

\subsection{Knowledge Spillover Channel}

The knowledge-spillover mechanism remains theoretically central, but the current sample provides limited direct empirical support because key mediators are not yet merged into the estimation panel. Recent evidence confirms that digital transformation can generate information spillovers across firms and market participants~\cite{WangWang2026}, and that spillover intensity may vary with strategic competition conditions in manufacturing settings~\cite{LiuGu2026}. A direct test in this setting requires mediator variables that map to the chain MSCI inclusion $\to$ external information intermediaries/investors $\to$ firm learning and implementation. In this context, analyst coverage (and optionally analyst forecast dispersion) and foreign institutional ownership are the preferred first-stage mediators. Because these variables are not yet integrated into the current merged panel, this study does not report a direct knowledge-channel coefficient and avoids proxy substitution that could blur causal interpretation.
...
Table~\ref{tab:cross_channel} demonstrates a clear ranking in empirical strength. In financing, two of three proxies are significant and suggest improved funding flexibility, while the composite KZ index remains imprecise. In governance, all three proxies are significant and theory-consistent, with interpretation centered on enhanced external monitoring conditions rather than guaranteed internal governance deepening. The deferred knowledge row is deliberately non-estimated and should be interpreted as a data-integration status marker rather than a null mechanism finding.
...
The combined evidence supports a positive but estimator-sensitive relationship between MSCI inclusion and digital-transformation outcomes. TWFE models produce large positive coefficients, but cohort-based modern DID diagnostics are markedly smaller and often imprecise. This divergence is consistent with heterogeneous treatment effects and potential TWFE weighting distortions under staggered adoption.

Accordingly, the central interpretation is not that liberalization generates a uniform large effect for all treated firms. Instead, the data suggest moderate and uneven treatment responses that vary across cohorts, contexts, and post-treatment horizons. The event-study dynamics remain informative because post-treatment effects accumulate rather than appearing as a one-period spike, which is more compatible with gradual strategic upgrading than pure announcement noise.

At the same time, the outcome measure is disclosure-based and should be interpreted as a hybrid indicator of strategic communication and substantive implementation. The robustness evidence shows directional consistency across alternative constructions, but cannot fully separate symbolic disclosure from realized operational transformation.

Mechanism evidence is similarly nuanced. Financing results are mixed and align more with financing-structure adjustment than broad constraint relief. Governance proxies are strong in sign and precision, but best read as evidence of stronger external monitoring and disclosure discipline, with compliance and passive-ownership channels still plausible. For knowledge spillovers, the study reports a transparent deferred-testing roadmap rather than over-interpreting proxy estimates.

\subsection{Implications for the Capital Market Liberalization Literature}

These findings extend the liberalization literature in a direction that has received limited attention. Most prior studies focus on financial outcomes---stock returns, liquidity, cost of capital---and treat real-side effects as secondary. The evidence here suggests that liberalization can influence how firms invest in technology and organizational capabilities, connecting financial integration to longer-term productivity-related behavior.
...
The results imply that capital-market liberalization can support digital upgrading, but only under clear complementary conditions. Treatment effects are stronger where absorptive capacity, institutional quality, and implementation readiness are already relatively high. Liberalization alone is therefore unlikely to generate uniform technology upgrading across firms.

For policymakers, this supports a conditional policy package: financial opening should be paired with governance reforms, digital infrastructure investment, innovation-financing support, and capability development in lagging regions and sectors. For firms, the evidence suggests that capital access is most useful when combined with organizational implementation capacity rather than disclosure expansion alone. For international investors, stewardship and monitoring can matter, but the strongest long-run gains are more likely when monitoring incentives translate into substantive, not merely symbolic, transformation commitments.
