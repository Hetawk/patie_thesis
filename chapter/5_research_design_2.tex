...
This chapter develops the computational measurement framework used to quantify digital transformation intensity from annual-report disclosures. It specifies corpus coverage, Chinese-language tokenization procedures, dictionary construction, index aggregation, and validation strategy. Following current text-as-data standards, measurement is treated as a core inferential component rather than a purely technical preprocessing step.

The chapter also treats measurement error as a first-order issue in causal analysis with textual outcomes. It therefore documents not only how the index is constructed, but also why each preprocessing and validation choice improves construct validity in a Chinese-language disclosure setting. By combining linguistic adaptation, domain-specific dictionaries, and multiple robustness specifications, the chapter shows that the estimated policy effects are not artifacts of a single coding rule, but stable patterns across plausible alternatives.

\section{Motivation: Why Text-Based Measurement for Digital Transformation?}

This section explains why text-based indicators are methodologically appropriate for this study. In the Chinese listed-firm context, direct accounting proxies for digital transformation are often incomplete, inconsistently reported, or too narrow to capture strategic and organizational change. Annual-report narratives provide broad firm-year coverage and enable multidimensional measurement, while downstream robustness checks help distinguish substantive transformation signals from disclosure-style noise.
