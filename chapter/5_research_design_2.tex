...
Annual reports are mandatory, standardized, and publicly available for all listed firms, which provides near-complete sample coverage without survey attrition. This makes computational text analysis feasible at scale in emerging markets where direct firm-year investment data are incomplete. At the same time, disclosure text combines substantive implementation information with strategic investor-facing communication. The resulting indicator should therefore be interpreted as a disclosure-based proxy for digital orientation and intensity, not as a one-to-one accounting measure of realized implementation. Methodologically, transparent preprocessing pipelines, reproducible dictionaries, and sensitivity checks across alternative constructions are used to improve interpretability under this hybrid-measure view.
...
\subsection{\texorpdfstring{Composite Index: $DT\_Comp = \log(1 + \Sigma\;\mathrm{keywords})$}{Composite Index: DT\_Comp = log(1 + sum keywords)}}
...
The text-based index is interpreted as a hybrid indicator that captures both substantive digital investment orientation and strategic disclosure behavior. This framing is important in the MSCI setting because inclusion may raise incentives to communicate modernization narratives to international investors. Construct-validity diagnostics therefore assess whether the disclosure-based signal remains economically informative, while acknowledging that it cannot fully separate rhetoric from implementation.

\subsection{\texorpdfstring{Correlation with IT Investment ($\rho = 0.67$)}{Correlation with IT Investment (rho = 0.67)}}

The composite $DT\_Comp$ index shows a Pearson correlation of $\rho = 0.67$ with firm-level IT investment (when available from supplementary CSMAR data fields). This provides supportive evidence that the text measure contains meaningful implementation-related information, while also implying that substantial variation remains outside direct IT-spending proxies.

\subsection{\texorpdfstring{Correlation with Digital Patent Applications ($\rho = 0.71$)}{Correlation with Digital Patent Applications (rho = 0.71)}}

The correlation with digital patent applications (patents in AI, software, and digital systems from CNRDS patent database) is $\rho = 0.71$. This cross-validation is consistent with genuine innovative activity, but it does not by itself rule out strategic disclosure incentives in annual-report narratives.

\subsection{Sensitivity Analysis Across Alternative Algorithmic Specifications}

We report results for eight alternative operationalizations of the dependent variable (Table~\ref{tab:alt_measures} in Chapter~\ref{chap:empirical_results}). These alternatives cover five groups. The first group contains alternative composite aggregations (DT\_Comp\_B). The second group contains application-domain sub-indices (DT\_App\_A, DT\_App\_B). The third group contains technology-specific measures (AI\_Tech, BigData\_Tech, Cloud\_Tech, and Blockchain\_Tech). The fourth group contains binary and standardized variants. The fifth group contains mention frequency versus intensity measures.
...
The computational index should be interpreted as a disclosure-based proxy that combines substantive and symbolic components. After MSCI inclusion, firms may strategically increase digital language to align with international investor expectations, even when implementation depth varies across firms.

Within these constraints, several findings suggest the measure remains economically informative rather than purely cosmetic: treatment effects show persistence in dynamic specifications, technology-specific patterns are stronger in domains with higher implementation requirements (especially AI and Big Data), and core results remain directionally stable after disclosure-length normalization and alternative outcome constructions.

Nevertheless, the current measure cannot fully separate strategic communication from realized implementation intensity. Future extensions should integrate direct substantive indicators such as digital patent shares, software copyright registrations, ERP/CRM procurement records, and machine-learning classification of implementation-detail disclosures versus strategic declarations.

These measurement choices provide the outcome architecture used in Chapter~\ref{chap:empirical_results} and preserve consistency with the identification design in Chapter~\ref{chap:research_design_1}.
