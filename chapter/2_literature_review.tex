...
Evidence from previous studies supports this logic.~\cite{Chen2022} demonstrate that digital transformation correlates with increased analyst coverage and better information accuracy.~\cite{DuWang2024} provide direct evidence that financial support enhances the innovation effects of digital transformation. Their findings show that the financial development context is important and that support from capital markets can be especially relevant in regions with more binding traditional constraints.~\cite{ChenZhang2024} and~\cite{Feng2024} expand this perspective to corporate financing structures.
...
Previous studies show that contextual heterogeneity is important. Effects differ depending on regional market development, ownership structure, and firm size~\cite{Zhang2024}. This heterogeneity indicates that average treatment estimates might hide significant distributional differences. Another recurring insight is that policy shocks interact with organizational absorptive capacity~\cite{Stiglitz2000}.
...
Recent studies have demonstrated that Chinese-language annual reports can provide credible NLP-based indicators of digital activity~\cite{Afzal2024}. This method relies on domain-cluster keyword extraction instead of counting generic buzzwords~\cite{Kumar2024}. NLP measurement is not meant to substitute financial indicators but enhances them~\cite{YLi2023}.
