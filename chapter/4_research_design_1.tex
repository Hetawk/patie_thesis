...
This chapter presents the causal identification strategy used to estimate the effect of capital market liberalization on firm-level digital transformation. It lays out treatment construction, sample design, model specification, and identification assumptions in a transparent sequence. In line with current empirical standards in applied economics and corporate finance, the chapter combines baseline DID estimation with dynamic diagnostics, matching-based checks, and explicit discussion of threats to internal validity.

The chapter is deliberately organized as a credibility protocol. Each design component is paired with the inferential risk it is intended to address, including selection on observables, differential pre-trends, timing heterogeneity, and disclosure-measurement artifacts. This structure helps readers assess not only the final estimator, but also the chain of assumptions and diagnostics behind the interpretation. In this sense, Chapter~\ref{chap:research_design_1} serves as the methodological backbone of the thesis.
