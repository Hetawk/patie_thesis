\chapter{Research Design~I: Identification Strategy for Capital Market Liberalization Effects}
...
Accordingly, the estimand is interpreted as the incremental effect of international capital-market access among firms that satisfy investability thresholds, rather than the effect of liberalization under fully random assignment across all listed firms.
...
MSCI inclusion provides a plausibly exogenous timing shock, but it is not equivalent to fully random treatment assignment. Inclusion criteria are rule-based---free-float market capitalization, liquidity thresholds, and Stock Connect accessibility---and the rollout schedule was enforced externally by MSCI, without direct control by individual firms. At the same time, these rules mechanically select firms that are typically larger, more liquid, and institutionally stronger before treatment. Therefore, identification is interpreted as quasi-experimental: it captures the effect of expanded global capital access for eligible firms under maintained assumptions, rather than a universal random-assignment effect across the entire A-share market. Following contemporary causal-design standards, we combine institutional argumentation with empirical diagnostics, including pre-trend checks, placebo assignments, and staggered-treatment estimators.
...
They also imply that findings should be interpreted as treatment effects for firms already near international investability standards, with limited external validity to firms far from those thresholds.
