...
\end{equation}
where $T_i$ denotes firm $i$'s inclusion year, and $k = -1$ is omitted as the reference period. Dynamic coefficients $\delta_k$ reveal whether digitalization accelerates precisely around the inclusion window and whether effects persist over time.
...
X_{it} = [\log(\mathrm{asset}_{it}),\; \mathrm{Age}_{it},\; \mathrm{ROA}_{it},\; \mathrm{ROE}_{it},\; \mathrm{Leverage}_{it},\; \mathrm{CashFlow/Asset}_{it},\; Q_{it},\; \mathrm{OwnConc}_{it},\; \mathrm{BoardSize}_{it},\; \mathrm{IndepDirPct}_{it}]
\end{equation}
